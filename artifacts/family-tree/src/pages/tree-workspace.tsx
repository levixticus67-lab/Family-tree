import { useState, useCallback, useEffect, useMemo } from "react";
  ...
  export default function TreeWorkspace() {
    ...
    // Do NOT destructure onNodesChange / onEdgesChange here.
    //
    // Root-cause of React error #185: @xyflow/react v12 uses an internal StoreUpdater
    // component (pZ) whose useEffect deps are ALL prop values. When we pass
    //   onNodesChange={onNodesChange}  (from useNodesState)
    // pZ syncs nodes to the Zustand store → xZ detects the change → calls
    // onNodesChange(changes) → applyNodeChanges returns a NEW array → setNodes →
    // re-render → pZ fires again → infinite loop → error #185.
    //
    // Removing onNodesChange / onEdgesChange from <ReactFlow> breaks this cycle.
    // Node drag is handled by onNodeDragStop (immediate optimistic setNodes +
    // server persist), so no UX regression.
    const [nodes, setNodes] = useNodesState([]);
    const [edges, setEdges] = useEdgesState([]);

    // Stable prop objects — prevent pZ's effect from refiring on every render
    // for props whose VALUES never change but whose references would be new inline.
    const fitViewOptions = useMemo(() => ({ padding: 0.15, minZoom: 0.3 }), []);
    ...
    const handleNodeDragStop = useCallback(
      (_: MouseEvent, node: Node) => {
        ...
        // Immediately commit the dragged position to React state so the node
        // does not snap back while the server round-trip is in flight.
        // (Without onNodesChange we no longer receive live position deltas from
        //  ReactFlow's internal store, so we apply the final position ourselves.)
        setNodes((nds) =>
          nds.map((n) =>
      ...
      [familyId, updateMember, qc, setNodes]
    );
